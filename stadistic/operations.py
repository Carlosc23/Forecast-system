...
def smoothing():
    ...
    for i in range(len(f2017)-1):
        d2017Suave.append(alpha*float(d2017M[i+1])+(1.0-alpha)*float(d2017Suave[i])) #ponderado junto con el del dia anterior
...
def averageM():
    ...
    with open("stadistic/csvfile.csv") as csvDataFile:
        ...
        for i in range(len(fecha)):
            if "2017" in fecha[i]:
                f2017.append(fecha[i])
                d2017.append(dinero[i])
            elif "2016" in fecha[i]:
                f2016.append(fecha[i])
                d2016.append(dinero[i])
        for j in range(len(f2017) - 3):
            if float(d2016[j + 1]) == 0:
                d2017.append(float(d2017[j + 2]))
            else:
                d2017.append((3 * float(d2017[j + 2]) + 10 * float(d2016[j + 1])) / 13.0)  # linea donde se calcula el promedio/media ponderada
            f2017.append(f2017[j + 1].replace("2017", "2017"))
        d2017.pop(0)
